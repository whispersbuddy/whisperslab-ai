...
export const CASE_STUDIES: CaseStudy[] = [
  ...
  },
  {
    ...
    detail: {
      ...
      howItWorks: [
        ...
        "The raw text, often in another language and sometimes with broken formatting, gets sent through an AI cleanup pass that rewrites the title and description in natural, correctly-translated language.",
  ...
  },
  {
    ...
    detail: {
      ...
      challenges: [
        "The contact lookup didn't always return a phone number or email for every homeowner, so the system had to handle those gaps gracefully instead of crashing the pipeline.",
        "Nothing could stop a contractor from getting the homeowner's contact info without paying first, so every preview had to prove the lead's value while keeping the actual contact details completely hidden until payment cleared.",
  ...
  },
  {
    ...
    detail: {
      ...
      challenges: [
        "Staff regularly forgot to add new contacts to the master list before those people emailed in, so the system needed a second, smarter way to still catch and correctly file those conversations.",
        "Inboxes and calendars were full of noise (internal chatter, marketing emails, automated meeting bots) that all had to be filtered out before anything reached the CRM.",
  ...
  },
  {
    ...
    detail: {
      ...
      challenges: [
        "The client specifically didn't want to add another paid software subscription, so everything had to run on tools they already had.",
  ...
  },
  {
    ...
    detail: {
      ...
      result: [
        "A task that used to eat an entire afternoon per inspection now finishes itself the moment the form is submitted, with every report matching the same premium layout, regardless of how many photos it contains.",
      ],
    },
  },
  {
    ...
    detail: {
      ...
      challenges: [
        "The system that accepts bulk removals was strict about exact wording, and a single lowercase letter in the wrong place caused every upload to be rejected.",
