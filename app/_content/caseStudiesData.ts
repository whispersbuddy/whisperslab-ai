...
// new: expanded, plain-language write-ups sourced from the underlying
// project doc, with client names left out (same anonymization the site
// has always used) and no new numbers invented beyond what's already on
// the card.
...
export const CASE_STUDIES: CaseStudy[] = [
  {
    ...
    detail: {
      ...
      situation: [
        "They were locked into an old platform's built-in document scanner — a proprietary \"black box\" that manually extracted numbers from uploaded bank statements. It couldn't be changed or improved, and someone still had to check its work by hand.",
      ],
      whatWeBuilt: [
        "A custom AI reader that opens every uploaded bank statement, finds the exact transaction data — amounts, dates, descriptions — and writes it straight into their database in a clean, structured format. It replaced the platform's built-in scanner entirely, with the AI's output locked to an exact structure every time, so the extracted data always lands in the right fields instead of just \"close enough.\"",
      ],
      challenges: [
        ...
        "The company's financial records were moving into a completely rebuilt, relational database at the same time, so the reader had to feed the new structure, not the old freeform one.",
  ...
  },
  {
    ...
    detail: {
      ...
      whatWeBuilt: [
        "A one-click system that takes the raw supplier feed, cleans up the text, translates it properly into the store's local language, and reformats every listing into clean, readable, on-brand copy — then pushes it straight to the live store. Along the way, we also fixed a checkout bug in the same pipeline: the store was silently rejecting any price with cents in it, quietly blocking real purchases.",
      ],
      challenges: [
